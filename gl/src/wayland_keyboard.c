...
static void touch_down_cb(void *data,
	     ...
	     wl_fixed_t y)
{
	...
	if(point->point_xcb){
		point->point_xcb(WL_POINTER_BUTTON_STATE_PRESSED,0,wl_fixed_to_int(x),wl_fixed_to_int(y));
	}
}

static void touch_up_cb(void *data,
		   struct wl_touch *wl_touch,
		   uint32_t serial,
		   uint32_t time,
		   int32_t id)
{
	...
	if(point->point_xcb){
		point->point_xcb(WL_POINTER_BUTTON_STATE_RELEASED,0,-1,-1);
	}
}

const struct wl_touch_listener toucher_listerner = {
		touch_down_cb,
		touch_up_cb,
		do_nothing,
		do_nothing,
